...
After a call of mirror(s);, the stack would store the following elements (underlined for
emphasis):

                    bottom [10, 50, 19, 54, 30, 67, 67, 30, 54, 19, 50, 10] top

Note that the mirrored version is added on to the top of what was originally in the stack. The
bottom half of the stack contains the original numbers in the same order. If your method is
passed an empty stack, the result should be an empty stack. If your method is passed a null
...
credit your code must run in O(n) time where n is the number of elements of the original stack.
...
        
        s2q(s, q); // queue in reverse
        
        // make stack in reverse
        // keep queue in reverse
        int size = q.size();
...
        }
        
        s2q(s, q); // queue in reverse + in order
        
        // make queue in order + reverse
        for (int i = 0; i < size; i++) {
            int num = q.remove();
            q.add(num);
        }
                
        q2s(q, s); // the above passed back into stack - the desired result
    }
}
